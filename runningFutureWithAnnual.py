...
diffSims = 100
evbmus_sim = np.nan*np.ones((len(mjoDatesSim),1000))
c = 0
for simIndex in range(diffSims):
    ...
    xds_MJOs_sim = xr_daily(xds_MJOs_sim,datetime(2022,6,1),datetime(2122,5,31))
    ...
    # reindex annual data to daily data
    xds_PCs_sim = xr_daily(xds_PCs_sim,datetime(2022,6,1),datetime(2122,5,31))
...
samplesPickle = 'dwtFutureSimulations1000.pickle'
outputSamples = {}
...
bmus_dates_months = np.array([d.month for d in mjoDatesSim])
bmus_dates_days = np.array([d.day for d in mjoDatesSim])
