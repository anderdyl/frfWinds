...
with open(r"waveWindHydrographs.pickle", "rb") as input_file:
...
for i in range(len(np.unique(bmuGroup))):
    ...
    if len(tempCopula) > 50:
        ...
        # else:
        kernels = ['GEV','KDE','KDE','KDE','KDE','KDE','GEV','KDE','KDE',]
    else:
        kernels = ['KDE','KDE','KDE','KDE','KDE','KDE','KDE','KDE','KDE',]
    samples = CopulaSimulation(tempCopula[:,0:9],kernels,125000)

    negIndex1 = np.where(samples[:,0] > 0.25)
    samples2 = samples[negIndex1]
    negIndex2 = np.where(samples2[:,1] > 0.05)
    samples3 = samples2[negIndex2]
    negIndex3 = np.where(samples3[:,2] > 3)
    samples4 = samples3[negIndex3]
    negIndex4 = np.where(samples4[:,3] > 2.5)
    samples5 = samples4[negIndex4]

    cutoff = 1.5*np.max(tempCopula[:,0])
    ...
    #     samples5 = samples5[toobig]
    # elif i == 41:
    #     toobig = np.where(samples5[:,0] < 5.8)
    #     samples5 = samples5[toobig]
    #
    # elif i == 49:
    #     toobig = np.where(samples5[:,0] < 10.5)
    #     samples5 = samples5[toobig]
    #
    # elif i == 55:
    #     toobig = np.where(samples5[:,0] < 11.5)
    #     samples5 = samples5[toobig]
    # elif i == 56:
    #     toobig = np.where(samples5[:,0] < 9)
    #     samples5 = samples5[toobig]
    # elif i == 59:
    #     toobig = np.where(samples5[:,0] < 12.4)
    #     samples5 = samples5[toobig]
    # elif i == 61:
    #     toobig = np.where(samples5[:,0] < 13)
    #     samples5 = samples5[toobig]
    # elif i == 65:
    #     toobig = np.where(samples5[:,0] < 7.2)
    #     samples5 = samples5[toobig]
    # elif i == 68:
    #     toobig = np.where(samples5[:,0] < 9.8)
    #     samples5 = samples5[toobig]
    # elif i == 69:
    #     toobig = np.where(samples5[:, 0] < 9.8)
    #     samples5 = samples5[toobig]
...
gevCopulaSimsPickle = 'gevCopulaSims125000.pickle'
outputgevCopulaSims = {}
